...
class CustomEnv:
    ...
    def __init__(self, df,control=0, initial_balance=1000, lookback_window_size=50, Render_range = 100):
        ...
        if control==1:
            ...
            self.Actor= Actor_Model(input_shape=self.state_size, action_space = self.action_space.shape[0], lr=self.lr, optimizer = self.optimizer)
            self.Critic = Critic_Model(input_shape=self.state_size, action_space = self.action_space.shape[0], lr=self.lr, optimizer = self.optimizer)
            self.Actor.Actor.load_weights("Crypto_trader_Actor.h5")
            self.Critic.Critic.load_weights("Crypto_trader_Critic.h5")
    ...
    def get_gaes(self, rewards, dones, values, next_values, gamma = 1.10, lamda = 0.95, normalize=True):
        omega=[(1-np.exp(-1*gamma*r**2)) for r, d, nv, v in zip(rewards, dones, next_values, values)]
        omega_mean=np.mean(omega)
        deltas = [r + omega_mean * (1 - d) * nv - v for r, d, nv, v in zip(rewards, dones, next_values, values)]
        deltas = np.stack(deltas)
        gaes = copy.deepcopy(deltas)
        for t in reversed(range(len(deltas) - 1)):
            gaes[t] = gaes[t] + (1 - dones[t]) * (omega_mean) * lamda * gaes[t + 1]
    ...
    def save(self, name="Crypto_trader"):
        ...
        # self.Actor.Actor.save_model(f"B_Actor.json")
        # self.Critic.Critic.save_model(f"B_Critic.json")
...
train_env = CustomEnv(train_df, control=0,lookback_window_size=lookback_window_size)
test_env = CustomEnv(test_df,control=0, lookback_window_size=lookback_window_size)

train_agent(train_env, visualize=False, train_episodes=100, training_batch_size=500)

# #----------------Replica World Data-------------------------------------------------------------
df = pd.read_csv('./price_data_replica.csv')
df = df.sort_values('Date')
...
train_env = CustomEnv(train_df, control=1,lookback_window_size=lookback_window_size)
# test_env = CustomEnv(test_df, control=1,lookback_window_size=lookback_window_size)
